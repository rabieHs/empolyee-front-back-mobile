...
class NotificationService {
  ...
  void _setupWebSocketConnection() async {
    try {
      ...
      print(
          'SOLUTION ROBUSTE: Configuration d\'un système de notification bidirectionnel');
      ...
      Timer.periodic(const Duration(seconds: 5), (_) {
        try {
          ...
          final url = '${ApiConfig.baseUrl}/notifications?t=$timestamp';
          ...
          http.get(
            ...
            if (response.statusCode == 200) {
              ...
              _notificationsController.add(notifications);
              print(
                  'SYNCHRONISATION: ${notifications.length} notifications récupérées');
            }
          }).catchError((error) {
    ...
    try {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          print(
              'Erreur lors de la récupération des notifications réelles - vérifiez la connexion au serveur');
          return [];
        }
      } catch (apiError) {
        if (kDebugMode) {
          print(
              'Erreur de connexion à l\'API pour les notifications: $apiError');
        }
        // En cas d'erreur de connexion, retourner une liste vide au lieu des données de démonstration
        print(
            'Erreur de connexion lors de la récupération des notifications réelles - vérifiez que le serveur est en cours d\'exécution sur le port 3002');
        return [];
      }
    } catch (e) {
      if (kDebugMode) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          getUserNotifications().then((notifications) {
            ...
            print(
                'Erreur lors de la mise à jour des notifications après marquage: $error');
          });
        } else {
          if (kDebugMode) {
            print(
                'Erreur lors du marquage de la notification comme lue: ${response.statusCode}');
            print('Corps de la réponse: ${response.body}');
          }
        ...
        if (kDebugMode) {
          print(
              'Erreur de connexion à l\'API pour marquer la notification: $apiError');
        }
  ...
  Future<void> clearAllNotifications() async {
    try {
      ...
      if (kDebugMode) {
        print(
            'Toutes les notifications ont été supprimées pour l\'utilisateur $userId');
      }
    } catch (e) {
      if (kDebugMode) {
