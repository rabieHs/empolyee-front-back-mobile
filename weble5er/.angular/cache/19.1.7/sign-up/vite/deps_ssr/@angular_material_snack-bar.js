...
var MatSnackBarContainer = class _MatSnackBarContainer extends BasePortalOutlet {
  ...
  enter() {
    if (!this._destroyed) {
      ...
      if (this._animationsDisabled) {
        this._renders.pipe((0, import_operators7.take)(1)).subscribe(() => {
  ...
  exit() {
    if (this._destroyed) {
      return (0, import_rxjs9.of)(void 0);
    }
    this._ngZone.run(() => {
...
var MatSnackBar = class _MatSnackBar {
  ...
  _attach(content, userConfig) {
    ...
    this._breakpointObserver.observe(Breakpoints.HandsetPortrait).pipe((0, import_operators7.takeUntil)(overlayRef.detachments())).subscribe((state) => {
